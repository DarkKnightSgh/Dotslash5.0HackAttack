...
#         return m
#         """
#     def download_and_save(self, tarDir="images", output_json_file="vizwiz_data.json"):
#         """
#         Download VizWiz images from vizwiz.org server and save the dataset as JSON.
#         :param tarDir (str): Target directory to save images.
#         :param output_json_file (str): Path to save the JSON file.
#         """
...
#             img_url = img_info['vizwiz_url']
#             img_filename = img_info['file_name']
#             img_path = os.path.join(tarDir, img_filename)
#             if not os.path.exists(img_path):
#                 print(f"Downloading image {img_id}...")
#                 urlretrieve(img_url, img_path)
#                 print(f"Image {img_id} downloaded.")

#         # Save the JSON content to a .json file
#         with open(output_json_file, "w") as json_file:
#             json.dump(self.dataset, json_file)

#         print("Images downloaded and JSON content saved successfully.")
# annotation_file = "/images/anno.json"
# vizwiz = VizWiz(annotation_file=annotation_file)
# vizwiz.download_and_save()
...
class VizWiz:
    ...
    def download_and_save(self, tarDir="/images", output_json_file="/images/anno.json"):
        if not os.path.exists(tarDir):
            os.makedirs(tarDir)
        ...
        for img_id in self.imgs:
            ...
            if not os.path.exists(fname):
                urlretrieve(img['vizwiz_url'], fname)
                downloaded_images.append(img_id)
            print('Downloaded {}/{} images'.format(len(downloaded_images), len(self.imgs)))
    
        json_data = {
        'annotations': list(self.dataset['annotations']),
        'categories': list(self.dataset['categories']),
        'images': list(self.dataset['images'])
         }
        json_file_path = os.path.join(tarDir, output_json_file)
        with open(json_file_path, 'w') as outfile:
            json.dump(json_data, outfile)
        print('Saved JSON annotations to', json_file_path)